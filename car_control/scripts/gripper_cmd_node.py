...
class GripperCmdNode(object):
    def __init__(self):
        self.velocity_topic = rospy.get_param('~velocity_topic', '/car_control/gripper_velocity')
        ...
        self.sub_vel = rospy.Subscriber(self.velocity_topic, Float64, self.vel_cb, queue_size=10)
        self.sub_open = rospy.Subscriber(self.open_close_topic, Bool, self.open_cb, queue_size=10)

        rospy.loginfo('gripper_cmd_node started: %s -> %s', self.velocity_topic, self.output_topic)
